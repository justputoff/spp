...
<div class="container-xxl flex-grow-1 container-p-y">
  <div class="card">
    ...
    <form action="{{ route('teacher.update', $teacher->id) }}" method="POST" class="p-3" enctype="multipart/form-data">
      @csrf
      @method('put')
      <div class="row mb-3">
        <label class="col-sm-2 col-form-label" for="basic-default-name">Alamat Guru</label>
        <div class="col-sm-10">
          <input type="text" name="address" class="form-control" id="basic-default-name" placeholder="" value="{{ $teacher->address }}" />
        </div>
      </div>
      <div class="row mb-3">
        <label class="col-sm-2 col-form-label" for="basic-default-name">No. HP Guru</label>
        <div class="col-sm-10">
          <input type="text" name="phone" class="form-control" id="basic-default-name" placeholder="" value="{{ $teacher->phone }}" />
        </div>
      </div>
